...
export class InviterService {
  ...
  chargeInvitation(id_praticien: number, id_activite_compl: number) {
    ...
    return this.http.get<Inviter>('http://127.0.0.1:8000/api/praticien/getUneInvitation/' +
  ...
  updateInviter(id_activite_compl: number, id_praticien: number, old_id_activite_compl: number) {
    ...
    this.http.post<Inviter>(`http://127.0.0.1:8000/api/praticien/updateInvitation/`
      , requestObject, {headers: headers})
      .subscribe(
        data => {
          this.inviter = new Inviter(data);
